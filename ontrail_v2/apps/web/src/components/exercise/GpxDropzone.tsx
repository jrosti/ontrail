...
export function GpxDropzone({
  onLoaded, onRemove, result,
  accent = 'var(--accent)',
  label, hint, removeLabel, loadedLabel, pointsLabel, filledLabel,
}: Props) {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          ...
          <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--accent)' }}>{loadedLabel}</span>
          ...
          >
            <Icon name="close" size={13} stroke={2.2} />{removeLabel}
          </button>
        </div>
        <div style={{
          fontSize: 12, color: 'var(--accent)', background: 'color-mix(in oklab, var(--accent) 10%, transparent)',
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 12, background: dragging ? 'color-mix(in oklab, var(--accent) 6%, transparent)' : 'var(--surface-2)',
        cursor: 'pointer', transition: 'all .15s', padding: '20px 16px',
      }}
    >
      <input
        ref={inputRef}
