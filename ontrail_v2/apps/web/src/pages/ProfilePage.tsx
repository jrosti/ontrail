...
import { updateProfile, exportData } from '../api';
  ...
  return (
    <main className="ot-page" style={{ maxWidth: 760, margin: '32px auto' }}>
      <div style={{ marginBottom: 18 }}>
        <h1 className="ot-page-title" style={{ margin: 0 }}>{t.editProfile}</h1>
      </div>

      <Card style={{ padding: 24, marginBottom: 22 }}>
        <h3 style={{ marginBottom: 18, fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 17 }}>
          {t.editProfile}
        </h3>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-dim)' }}>{t.displayName}</span>
            <input
              className="ot-input"
              value={displayName}
          ...
          <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-dim)' }}>{t.synopsisField}</span>
            <textarea
              className="ot-input"
              value={synopsis}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-dim)' }}>{t.restHr}</span>
            ...
            <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-dim)' }}>{t.maxHr}</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <input
                  className="ot-input"
                  type="number"
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginTop: 4 }}>
            ...
            >
              {saveMutation.isPending ? '…' : saved ? (lang === 'fi' ? 'Tallennettu!' : 'Saved!') : t.saveProfile}
            </button>
            <button
              className="ot-iconbtn"
              style={{ width: 'auto', padding: '0 14px', fontSize: 14, fontWeight: 600, color: 'var(--text-dim)' }}
      ...
      <Card style={{ padding: 24 }}>
        <h3 style={{ marginBottom: 16, fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 17 }}>
