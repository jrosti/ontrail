...
const SPORT_PROFILES = [
  ...
  { key: 'bike',             weight: 20, distM: [35000, 15000], durSec: [5400, 1800], hr: [145, 10], climb: 400 },
  ...
  { key: 'maastojuoksu',     weight: 5,  distM: [10000, 4000], durSec: [3600, 900], hr: [158, 12], climb: 250 },
  { key: 'maantiepyoraily',  weight: 5,  distM: [60000, 20000], durSec: [7200, 1800], hr: [148, 8], climb: 600 },
  { key: 'ski',              weight: 4,  distM: [15000, 8000], durSec: [5400, 1800], hr: [150, 10], climb: 300 },
  { key: 'voimaharjoittelu', weight: 4,  distM: [0, 0],       durSec: [3600, 600],  hr: [125, 12], climb: 0 },
];

const TITLES: Record<string, string[]> = {
  run:              ['Aamuverryttely', 'Helppo hölkkä', 'Intervalli', 'Pitkäjuoksu', 'Iltajuoksu', 'Perjantaijuoksu', 'Tempojuoksu', 'Rauhallinen lenkki'],
...
function weightedPick() {
  const total = SPORT_PROFILES.reduce((s, p) => s + p.weight, 0);
...
async function main() {
  ...
  const exercises: {
    sport: string; title: string; date: string;
    durSec: number; distM: number | null; avgHr: number | null; climbM: number | null;
    tags: string[];
  }[] = [];
  ...
  for (let d = 0; d < days; d++) {
    ...
    const climbM = profile.climb > 0 && distM ? Math.round(gauss(profile.climb, profile.climb * 0.4)) : null;
